...
extern s8 L_C_flag;
extern u8 id_num,tempshuzhi_1,tempshuzhi_2;
...
void ADC3_CH2_DMA_Config_capacitor_A(void);
void ADC2_CH3_DMA_Config_capacitor_B(void);
void ADC1_CH9_DMA_Config_capacitor_C(void);


u16 ADC_Temperature(u8 temp);
...
//status_dis_node dis_list[2];
 ...
 void	warn(void);
void current_capacitor_ABC (void);
...
DMA_InitStructure.DMA_Channel = DMA_Channel_0;  
 ...
 GPIO_InitStructure.GPIO_Pin = GPIO_Pin_1|GPIO_Pin_2|GPIO_Pin_3|GPIO_Pin_4|GPIO_Pin_7|GPIO_Pin_5|GPIO_Pin_6;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AN;
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_NOPULL ;
		...
		void USART3_IRQHandler(void)
{
	...
	{	
		 RS485_RX_BUF[RS485_RX_CNT++]=USART_ReceiveData(USART3); 	//读取接收到的数据
...

{
   if(mybox.myid==tx_r485[1])//判断是否是发给本机的信息或是广播信息
   	{
if((tempshuzhi_1<70)&&(temperature_warn==0))//自身温度是否正常 
{
	if( tx_r485[2]==1) 
	{
	 if( tx_r485[4]==0)
...
{
tempshuzhi_1=(u8)(258-((ADC_Temperature(1)*255)/4096));

tempshuzhi_2=(u8)(258-((ADC_Temperature(2)*255)/4096));
...
/************从机功能 温度报警************************/
{
if((tempshuzhi_1>=temperture_w||tempshuzhi_2>=temperture_w)&&temperature_warn==0&&auto_on==1)
{
 if(	status_box.work_status[0]==1)
		   {while(subswitchABC_onoff(1,0,1)==0)break;}		  //投
                 if(	status_box.work_status[1]==1)
...
if(tempshuzhi_1<=temperture_w-5&&tempshuzhi_2<=temperture_w-5&&temperature_warn==1)
{temperature_warn=0;
status_box.work_status[0]=3;
status_box.work_status[1]=3;
status_box.work_status[2]=3;
...
{
u16 i;
u16 CA_max=0,CB_max=0,CC_max=0;
{
ADC_Converted_VValue=0;
ADC_Converted_CValue=0;
ADC3_CH2_DMA_Config_capacitor_A();
 ADC1_CH9_DMA_Config_capacitor_C();
 for(i=0;i<TEST_LENGTH_SAMPLES/2;i++)
	 	{
	 	
if(CA_max<(u32)((ADC_Converted_VValue)))CA_max=(u32)((ADC_Converted_VValue));///  1550

if(CC_max<(u32)((ADC_Converted_CValue)))CC_max=(u32)((ADC_Converted_CValue));///  1550
...
delay_us(36);//36->512
 ...
 ADC_Converted_VValue=0;
 ADC2_CH3_DMA_Config_capacitor_B();

  for(i=0;i<TEST_LENGTH_SAMPLES/2;i++)
	 	{
	 	

if(CB_max<(u32)((ADC_Converted_VValue)))CB_max=(u32)((ADC_Converted_VValue));///  1550
...
delay_us(36);//36->512
 	...
 	dianliuzhi_C_A= 0.0073*CA_max-10.3;
	dianliuzhi_C_B= 0.0073*CB_max-10.3;
	dianliuzhi_C_C= 0.0073*CC_max-10.3;

dianliuzhi_C_A= dianliuzhi_C_A*3.6;
	dianliuzhi_C_B= dianliuzhi_C_B*3.6;
	dianliuzhi_C_C=dianliuzhi_C_C *3.6;
